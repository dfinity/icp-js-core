...
describe('makeActor', () => {
  ...
  it('should enrich actor interface with httpDetails', async () => {
    ...
    jest.spyOn(httpAgent, 'update').mockImplementation(async (cid, fields) => {
  ...
  it('should properly handle preSignReadStateRequest option', async () => {
    ...
    // Spy on update to track calls
    const mockCallAndPoll = jest.spyOn(httpAgent, 'update').mockResolvedValue({
      certificate: undefined as unknown as certificateImport.Certificate,
      rawCertificate: new Uint8Array(),
      reply: expectedReplyArg,
    ...
    // Verify update was called with preSignReadStateRequest option
  ...
  it('should allow method-specific pollingOptions override with withOptions', async () => {
    ...
    // Verify update was called with the right options
    expect(mockCallAndPoll).toHaveBeenCalledTimes(1);
...
describe('queryStrategy', () => {
  ...
  function makeMockFetch() {
    return jest.fn((resource: string | URL | Request) => {
      if (resource.toString().endsWith('/call')) {
        return Promise.resolve(
          new Response(null, {
            status: 202,
  ...
  describe('queryStrategy', () => {
    ...
    it('sends query methods through the update path when queryStrategy is update', async () => {
      ...
      const updateMock = jest.spyOn(httpAgent, 'update').mockResolvedValue({
        certificate: undefined as unknown as certificateImport.Certificate,
        rawCertificate: new Uint8Array(),
        reply: expectedReplyArg,
        callResponse: { ok: true, status: 202, statusText: 'accepted', body: null, headers: [] },
    ...
    it('sends query methods through the query path by default', async () => {
      ...
      const updateMock = jest.spyOn(httpAgent, 'update');
      const actor = Actor.createActor(actorInterface, {
        canisterId,
        agent: httpAgent,
      });
